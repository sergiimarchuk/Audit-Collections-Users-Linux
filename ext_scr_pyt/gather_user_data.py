...
def log_rm():
    ...
    if os.path.isfile(f"{cwd}/{checklog_file_}"):
        os.remove(f"{cwd}/{checklog_file_}")
...
def log_rm_ex(file_path, archive_dir='/tmp'):
    ...
    if os.path.isfile(file_path):
        ...
        #print(f"File {file_name} has been moved to {archive_path}")
        return True
    else:
...
def checklog_file(content):
    ...
    with open(checklog_file_, 'a') as f:
        f.write(content + '\n')
